...
const editModal: React.FC<ModalProps> = ({
  ...
  const getCurrentDateTime = (param?: 'date' | 'time', date?: string): string => {
  ...
  const [AddTagToTask] = useServicesAPI({
    endpoint: 'task/x/tag/x',
    method: RequestMethod.POST,
    testId: 'add-tag-to-task'
  })
  const [RemoveTagFromTask] = useServicesAPI({
    endpoint: 'task/x/tag/x',
    method: RequestMethod.DELETE,
    testId: 'remove-tag-from-task'
  })
  ...
  const handleModalContentClick = (e: { stopPropagation: () => void }): void => {
    e.stopPropagation()
  }

  const handleAddTag = (tag: tagsProps): void => {
